...
for YEAR in $(seq 2015 2021); do
	for TEST_TYPE in I II; do
		for PROBLEM in $(seq 5); do
			...
			echo "${YEAR} AIME ${TEST_TYPE} #${PROBLEM}" >> aime/easy
		done
		...
		for PROBLEM in $(seq 6 10); do
			...
			echo "${YEAR} AIME ${TEST_TYPE} #${PROBLEM}" >> aime/medium
		done
		...
		for PROBLEM in $(seq 11 13); do
			...
			echo "${YEAR} AIME ${TEST_TYPE} #${PROBLEM}" >> aime/hard
		done
		...
		for PROBLEM in $(seq 14 15); do
			...
			echo "${YEAR} AIME ${TEST_TYPE} #${PROBLEM}" >> aime/veryhard
		done
	done
done
...
for YEAR in $(seq 2014 2020); do
	for PROBLEM in $(seq 15); do
		...
		echo "${YEAR} AMC 8 #${PROBLEM}" >> amc8/easy
	done
	...
	for PROBLEM in $(seq 16 18); do
		...
		echo "${YEAR} AMC 8 #${PROBLEM}" >> amc8/medium
	done
	...
	for PROBLEM in $(seq 19 22); do
		...
		echo "${YEAR} AMC 8 #${PROBLEM}" >> amc8/hard
	done
	...
	for PROBLEM in $(seq 23 25); do
		...
		echo "${YEAR} AMC 8 #${PROBLEM}" >> amc8/veryhard
	done
done
...
for YEAR in $(seq 2015 2021); do
	for TEST_TYPE in A B; do
		for PROBLEM in $(seq 15); do
			...
			echo "${YEAR} AMC 10${TEST_TYPE} #${PROBLEM}" >> amc10/easy
		done
		...
		for PROBLEM in 16 17 18; do
			...
			echo "${YEAR} AMC 10${TEST_TYPE} #${PROBLEM}" >> amc10/medium
		done
		...
		for PROBLEM in 19 20 21 22; do
			...
			echo "${YEAR} AMC 10${TEST_TYPE} #${PROBLEM}" >> amc10/hard
		done
		...
		for PROBLEM in 23 24 25; do
			...
			echo "${YEAR} AMC 10${TEST_TYPE} #${PROBLEM}" >> amc10/veryhard
		done
	done
done
...
for YEAR in $(seq 2015 2021); do
	for TEST_TYPE in A B; do
		for PROBLEM in $(seq 15); do
			...
			echo "${YEAR} AMC 12${TEST_TYPE} #${PROBLEM}" >> amc12/easy
		done
		...
		for PROBLEM in $(seq 16 18); do
			...
			echo "${YEAR} AMC 12${TEST_TYPE} #${PROBLEM}" >> amc12/medium
		done
		...
		for PROBLEM in $(seq 19 22); do
			...
			echo "${YEAR} AMC 12${TEST_TYPE} #${PROBLEM}" >> amc12/hard
		done
		...
		for PROBLEM in $(seq 23 25); do
			...
			echo "${YEAR} AMC 12${TEST_TYPE} #${PROBLEM}" >> amc12/veryhard
		done
	done
done
